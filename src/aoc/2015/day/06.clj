...
(defn row-off [row range']
  (letfn [(turn-off [[a b :as cell] [start end]]
            (if (or (< b start) (> a end))
              [cell]
              (filter valid-range? [(list a (dec start))
                                    (list (inc end) b)])))]
    (reduce (fn [acc cell]
              (into acc (turn-off cell range')))
            []
            row)))

(defn row-toggle [row range']
  (loop [pre []
    ...
    (cond
      ...
      :else
      (let [rem (when (> end b) (list (inc b) end))
            pre' (into pre (filter valid-range?
                                   [(list start (dec a))
                                    (list a (dec start))
                                    (list (inc end) b)]))]
        (if rem
...
(def final-state
  (reduce (fn [acc [action y range']]
            (let [row (get acc y)
                  row' (case action
                         :on (row-on row range')
                         :off (row-off row range')
                         :toggle (row-toggle row range'))]
              (assoc acc y row')))
          (vec (repeat 1000 []))
          per-row-instructions))

(defn count-lights [rows]
  (->> rows
       (mapcat identity)
       (reduce (fn [acc [a b]]
                 (+ acc (inc (- b a))))
