;; Today is March 1, 2022, and I'm starting on day 19 of last
;; year's AoC; we'll see when I finish.
...
(s/fdef set-orientations
  :args (s/cat :coords (s/coll-of ::coord))
...
(st/instrument)
(rcf/tests
  (sequential? (set-orientations [[1 2 3] [4 5 6]])) := true)
...
(set-orientations (parse-scanner "--- scanner 0 ---\n-1,-1,1\n-2,-2,2\n-3,-3,3\n-2,-3,1\n5,6,-4\n8,0,7"))
...
;; TODO! Bad!
(def scanner-locs (atom []))
(defn adjusted-scanner [scanner-a scanner-b]
  (let [[sorted-a sorted-b] (map sort [scanner-a scanner-b])
    ...
    (first (for [d candidate-distances
                 :let [
                       translated-b (map #(translate % d) sorted-b)
                       overlap (set/intersection (set sorted-a) (set translated-b))
                       do-overlap (>= (count overlap) 12)]
                 :when do-overlap]
             (do
               (swap! scanner-locs conj d)
               translated-b)))))

(defn find-adjusted-scanner
  ;"Returns scanner-b's position relative to scanner-a's,
  ;if such can be determined; otherwise nil."
  [scanner-a scanner-b]
  (let [orientations (set-orientations scanner-b)]
    (first (keep #(adjusted-scanner scanner-a %)
                 orientations))))

(defn all-beacons [scanners]
  (let [[true-scanner & more-scanners] scanners]
    (loop [beacons (set true-scanner)
           scanners more-scanners]
      (let [scanner-beacons (map #(vector % (find-adjusted-scanner beacons %))
                                 scanners)
        ...
        (if (not-empty new-beacons)
          (recur (into beacons new-beacons)
                 unmatched-scanners)
          beacons)))))
...
(rcf/tests
  (count (all-beacons test-scanners)) := 79)

(comment
  (println (count (all-beacons test-scanners)))

  (println (count (all-beacons scanners)))
